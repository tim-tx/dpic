...
\subsection{SVG text\label{SVGtext:}}
  ...
  renderer back to \pic interpreter is established, there is need
  of a mechanism for the \pic interpreter to set the size of text,
\begin{wrapfigure}[9]{r}{0.3\textwidth}%
  \vspace*{-\baselineskip}%
  \hspace*{1em}%
  \parbox{0.3\textwidth}{\input{Xtest.tex}}%
 \end{wrapfigure}%
  assuming plain text only without significant mathematics, for example.
  ...
  Consequently,
  a language issue arises: if the attribute {\tt ht {\it y}}
  is to be used to set text size,
  should the value of {\it y} be the nominal height (the point size) or
  the formatted height of the text? Initial versions of the experimental
  SVG output of \dpic used the
  nominal height but later versions now conform to \LaTeX usage
  by assuming that {\it y} is the
  true graphical height in drawing units.  (This might require
  minor changes to legacy diagrams but the changes are usually small.)
  ...
  of
    \ntt{dpPPI = 96}
    \ntt{dptextratio = 0.66}\\
  in the outermost scope of a diagram.  These are ordinary variables
  that can be referenced as usual or changed by, for example,
    \ntt{dpPPI := {\it value}}.
  ...
  This parameter can be reset using the assignmnent
    \ntt{dpPPI := {\sl value}}\\
  for different screen resolution. 

  When \dpic sees the {\tt ht {\it y}} attribute attached to SVG text,
  it sets the text size using
  the variable {\tt dptextratio} to estimate the nominal height
  required.  Otherwise \dpic uses the current value of the environment
  variable {\tt textht}, which has default value $11/72 \times 0.66,$
  to set the size to 11\,bp.

  By default, \dpic sets the width of text
  to the current value of {\tt textwid} but,
  ...
  In principle, this might be automated using suitable macros and scripting
  (JavaScript, probably).
