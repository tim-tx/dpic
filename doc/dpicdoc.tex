...
\iffalse
For a summary of dpic syntax, Appendix~A contains a formatted Unix man
page for dpic, and Appendix~B contains the dpic grammar.
\fi
...
\begin{tabbing}
\quad\=\hbox{(none)\ }\= \kill
\> (none) \> Latex picture output (very limited font-based drawing commands)\\
\> {\tt -d} \> PDF output \\
\> {\tt -e} \> pict2e output \\
...
\> {\tt -v} \> SVG output \\
\> {\tt -x} \> xfig 3.2 output \\
\> {\tt -z} \> safe mode ({\tt sh}, {\tt copy}, and print to file disabled)
\end{tabbing}
...
\iffalse
Their differences therefore remain constant, with one exception:
if the dpic {\tt scaled {\sl x}} construct is employed, the trick has
to be modified as shown:
\ntt{s = 0.7}
\ntt{Q: [ v = 5; w = 6}
\ntt{\quad Origin:\ (0,0); Export:\ (v,w) ] scaled s}
\ntt{v = (Q.Export.x - Q.Origin.x)/s}
\ntt{w = (Q.Export.y - Q.Origin.y)/s}
\fi
...
allow nesting:
% (As usual, commas within the arguments must be in strings or
% protected by~{\tt(~)} parentheses.):
\ntt{ld\_\_ = 0}
...
\iffalse
\subsection{Arbitrary filled paths}
  The unaugmented pic language and its gpic extensions do not allow the
  filling of arbitrary closed curves other than boxes, circles, and
  ellipses.  However, the recent outlined and shaded attributes allow
  the filling of areas outline by one linear object such as a
  multi-segment line or spline, as described above.
  Use the \verb|\pscustom| function of PSTricks, for example,
  for more elaborate fills.
\fi
...
\end{verbatim}
allow the command
\ntt{circle shaded "gold"}\\
to work for the three different postprocessors specified plus
direct postscript or PDF output.
Alternatively, the \Mfour\ macro
{\tt rgbstring({\sl fraction,fraction,fraction})}
evaluates to a string compatible with the postprocessor
so that the following produces the same result:
\ntt{circle shaded rgbstring(1,0.84,0)}.

  More elaborate options can also be invoked. For example, with PSTricks
  output the sequence
...
arcs.
% The language was never explicitly designed to draw filled objects
% of arbitrary shape.
...
\subsection{Logic}
  ...
  parentheses may sometimes be in order to guarantee identical behavior.
  In addition, put string comparisons in parentheses, e.g.
  \ntt{("{\sl text1}" == "{\sl text2}")}
...
% In other cases, putting a comparison using {\tt<} inside parentheses will
...
% Gpic and ATT pic use {\tt .start} for all of the compass corners of these
% objects, whereas dpic attempts a slightly more elaborate computation.
  The compass corners of multisegmented linear objects are not precisely defined
  and they should be used with care.

% Be explicit in placing objects rather than relying on defaults.
...
% Note the ambiguity in the pic language:
...
arguments can be arbitrary expressions rather than the constants shown.
  ...
  quote character in a string, so {\tt"\tBS""} is a length-one string containing
...
%"{\$}1", "$\$ 1$", or "${0}$"
  ...
  for example.
%  Some previous versions of dpic allowed {\tt\$} to escape the
%  dollar sign in strings but this created other difficulties.

  Both dpic and gpic allow logical comparison of strings.
  Put the comparison in parentheses.

  String height and width are unscaled on final output from dpic since
  these depend on later formatting by LaTeX.
...
\subsection{Scaling}
  ...
  in place of setting the scale environment variable when, for example,
  different parts of a diagram require different scaling.  As always,
  line thicknesses are not scaled.
...
# Method 2 (change only the currently drawn object):
  arrow thick 2 ht 0.18 wid 0.09 -> 0
\end{verbatim}
...
\subsection{{\tt continue}}
  ...
  {\tt then}:
   \ntt{move to (0,0); line right 1; then to (Here,(2,1))}\\
  The keyword {\tt continue} would not have been necessary
...
\subsection{Subscripted variables and locations}
  ...
  Thus {\tt Pos[{\sl expression}]} is a valid name for a position.  The rounded
  integer value of the expression is used.  Similarly,
...
\subsection{Macros}
  ...
  invoked as
    \ntt{A: spot((1,2))}\\
  is expanded by gpic to
    \ntt{A: circle at (1,2)}\\
  whereas it is expanded literally by dpic to
    \ntt{A:}
      \ntt{\quad circle at (1,2)}\\

  (Gpic ignores the white space after the \tLB\ but not before the \tRB).
  To emulate the gpic behaviour, write
    \ntt{define spot \tLB\ }\tBS
      \ntt{\quad circle at \$1}
    \ntt{\tRB}\\
  or simply use
    \ntt{define spot \tLB\ circle at \$1 \tRB}\\
  To allow, for example,
    \ntt{A: spot((1,2)) dashed}\\
  use the last definition above or write
    \ntt{define spot \tLB\ \tBS}
      \ntt{\quad circle at \$1 \tBS}
    \ntt{\tRB}
\fi
...
\subsection{PSTricks anomaly}
  Some time in the mid 2000 to 2010 interval,
